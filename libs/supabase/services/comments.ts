...
export default new (class CommentService extends BaseDbService {
  private _getChildren (rows: Comment[], parentId: number): ProductComment[] | undefined {
    return rows.filter(i => i.parent_id === parentId).map(i => ({
      ...i,
  ...
  async getByProductId (productId: number): Promise<ProductComment[] | null> {
    ...
    return data.filter(i => i.parent_id === null).map(i => ({
      ...i,
