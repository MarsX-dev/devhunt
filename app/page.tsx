...
function RenderDatePart({number, letter}) {
    return (
        <div className="rounded-md p-[1px] overflow-hidden bg-gradient-to-b from-[#514b6130] to-[#514b6100]">
            <div
                className="py-2 px-3 rounded-md w-11 leading-4 flex items-center justify-center bg-gradient-to-b from-[#51269c40] to-[#DBB8BF10] backdrop-blur-md">
...
const banner = (
    <div
        className="flex border rounded-xl border-slate-800 bg-slate-800 p-5 flex-col gap-1 md:gap-2 items-center justify-end">
...
export default async function Home() {
    ...
    function weekTools(group) {
        return <>
            ...
            <div className="mt-3 text-slate-400 text-sm">❗ Vote for your favorite <b className='text-orange-400'>↳</b>
            </div>
    ...
    return (
        <section className="max-w-4xl mt-5 lg:mt-10 mx-auto px-4 md:px-8">
            ...
            <div className="mt-10 mb-12">
                {
                    launchWeeks.map((group, index) => index > 0
                        ? prevWeekTools(group)
                        : weekTools(group))
                }
            </div>
